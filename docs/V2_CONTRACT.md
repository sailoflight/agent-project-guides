...
Draft proposals use exclusive creation in clone-local state. A declared non-author reviews them, and review/promotion remain bound to the proposal's project digest. Promotion acquires the shared project mutation lock, rereads the descriptor, and atomically publishes only a missing, descriptor-bound, non-symlink memory target through no-replace linking; existing records are never conditionally overwritten. Updates use a new ID and may supersede only a record carrying fully validated reviewed provenance for the same project; the digest recorded in that record names the descriptor epoch that produced it and must be well formed rather than equal to the current digest, so a later descriptor never makes promoted memory permanently unreplaceable. Promotion never stages or commits, and purge removes local proposals only.
