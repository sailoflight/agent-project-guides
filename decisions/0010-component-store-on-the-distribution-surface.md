...
entry was written here and the store remained packages-only until D12.**
...
**D12 — The first service entry is written, and it can only ever be `degraded`.** The store now holds `services/am.json`: `am` on `127.0.0.1:18765`, `/health`, `transport: http`, `revision: 0.3.36`, `singleton: true`, `delivery: staged`, `stop: signal`, and `asserted_identity: true`. D9 is why it can exist honestly - `identity` and `asserted_identity` are mutually exclusive, and `am`'s identity lives only in the MCP handshake, so the record asserts a revision instead of checking one. The consequence is deliberate and permanent: `probe` reports it `degraded` and it can never be `available`, because nobody can check who answers. This is the first entry the store did not refuse to describe, and it removes the "packages-only" caveat without weakening a single state: an asserted identity is exactly what `degraded` means. D6 and D8's "no service entry was written" are historical as of this decision.

Two measured limits belong with it. First, in this environment a loopback port with no listener **times out** rather than refusing (`curl --noproxy '*' http://127.0.0.1:9/` returns exit 28, not `ECONNREFUSED`), so the `not-installed` state is not observable here and a dead declared service surfaces as `degraded` with `the health probe timed out`. The direction is conservative: absence can be reported more vaguely than it is, never as `available`. Second, the acquisition side stays outside the distribution surface (D4) and is written down only in `scripts/build-component-store.mjs` and its `--help`, together with the per-component configuration checklist in plan §13.22.1 - including the prerequisites that are deliberately **not** expressible (`br`/`bv`'s `.beads` directory, `slb`'s project init, `sbh`'s daemon plus root, and the variable name of `cass`'s API key, which no measurement established).
