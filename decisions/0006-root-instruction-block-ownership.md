...
|---|---|---|---|---|
| `br` (`beads_rust`) | `<!-- br-agent-instructions-v1 -->` / `<!-- end-br-agent-instructions -->` | **yes** — `AGENTS.md.bak` (`src/cli/commands/agents.rs:660`) | walks upward to `.git`/`.beads` (`:348-399`) | scan + **real run** |
| `bv` (`beads_viewer`) | `<!-- bv-agent-instructions-v{n} -->` / `<!-- end-bv-agent-instructions -->`; code emits **v7** (`pkg/agents/blurb.go:18-23`) | **no** — atomic `os.Rename` | append/update/remove need an existing file; create does not | scan + field files |
| `ee` (`eidetic_engine_cli`) | `<!-- ee:agentsmd:begin generation=N hash=H -->` / `<!-- ee:agentsmd:end -->` | **yes** — `AGENTS.md.ee-backup` (`src/core/agentsmd.rs:1391`) | in-place region rewrite; `--create` to make the file | scan |
| `slb` | `<!-- slb:cursor-rules:start -->` / `<!-- slb:cursor-rules:end -->` (`internal/cli/integrations.go:73`) | **no** | upsert; **preview by default** | scan |
| `sbh` (`storage_ballast_helper`) | `<!-- sbh-docs:begin <section> -->` / `<!-- sbh-docs:end -->` (`src/cli/docs.rs:2450`) | **no** | rewrites an existing region only | scan + field files |
| `frankenterm` | `<!-- frankenterm:start -->` / `<!-- frankenterm:end -->`; plus inline `<!--count:{NAME}-->VALUE<!--/count-->` (`scripts/stamp-readme-counts.sh:140`) | **yes** for the config writer — `.ft-agent-config-<32hex>.backup`; **no** for the count stamper | explicit file list | scan |
| `am` (`mcp_agent_mail_rust`) | `<!-- am:blurb -->` / `<!-- am:blurb:end -->`, **no version token** | **no** — raw `std::fs::write` (`crates/mcp-agent-mail-cli/src/lib.rs:89815`, `:89838`) | **recurses three levels** (`max_depth = 3`, `:89638`); needs the file to exist | scan |
...
| `cass` (`cass_memory_system`) | `<!-- Auto-generated rules from cass-memory playbook -->` **and** `<project_rules>` / `</project_rules>` (`playbook.ts:665-686`) | **no** (`.tmp.<hex>` only) | rewrites the whole file, refuses if it exists without `--force` | scan |
| `ntm` | **no HTML comment** — region is `<INSTRUCTIONS>` / `</INSTRUCTIONS>` (`internal/cli/agents_template.go:28,95`) | **no** | whole-file template; creates only | scan + field files |
