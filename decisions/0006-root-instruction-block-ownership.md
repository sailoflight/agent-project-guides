...
- **There is no backup**: `grep -n 'backup\|\.bak\|cp -' scripts/install.sh` and the same search in `manage-root-blocks.mjs` return nothing. A bad merge overwrites the previous root file.
...
Three independently developed tools arrived at the **same** protocol from the same pressure: one marker-delimited managed region per writer, never touch bytes outside it, refuse rather than guess, be idempotent, and detect hand edits of your own region. The differences are equally informative — `ee` carries a **content hash and a generation inside its marker** and **writes a backup**, and APG does neither.
...
Validation: `scripts/test-install.sh` (managed-prefix routing, recoverable CLAUDE scope transactions, exact aliases, project profiles, MCP subtypes, cloud freshness, state lifecycle, safety guards), `scripts/validate-routing.mjs`, the P5 interop sequence in `.agent-scratch/external-test/`, and catalog/manifest regeneration after any change to shipped content. Signals for review: an external writer that stops appending, an install that relocates content found above the prefix, a root file mutation without a backup, or the root block exceeding its budget.
