...
3. Natural-language `apg context` is deterministic and LLM-free. Explicit plane/role/mode inputs bypass inference. Protected or materially ambiguous work returns selected-view choices with stable IDs, exact routes, matched rules, conflict reasons, and executable next commands; unavailable roles are reported only as required expansion. Candidate roles, profiles, and generic safety documents are never union-loaded.
4. Context includes mandatory authority before optional route content and fails if the larger exact JSON/direct-context estimate exceeds the aggregate budget. Existing per-subject section budgets remain authoritative. Shared generation continuity and choice selection use an installation-state HMAC rather than a caller-recomputable checksum. Route resolution never grants production authority.
...
6. The materializer supports fresh consumer projects plus reviewed schema 1 adoption. It previews by default and mutates only with `--apply` or `v3-apply`. Apply uses a project mutation lock, durable journal, transition-blocked root marker, staged guide tree, ordered descriptor/tree/root publication, final validation, and immutable receipts.
7. Migration from descriptor schema 1 is `preview -> digest-bound apply -> rollback`. Apply rebuilds the candidate, requires exact unchanged preimages, binds recovery bytes into the descriptor, and reports transitional containment while legacy recovery remains. Rollback preflights every owned postimage and stops with zero writes on ambiguity. Finalization remains deferred.
...
10. Compatible channels, selected CLI, project CLI, self-contained, source-worktree schema 2, full-local dangerous, and migration finalization are deferred.
