...
| 9 | `docs/memory` 记录内嵌 `project_digest` 全面变旧 | **18/18 现已全部落后于当前描述符**（3.0.9 时是 18/18，之后每次升版只会加剧）；`finding.h1.bootstrap-token-only-validation` 仍是 `state: promoted` 且 trigger/residual 是旧文本，**无法 supersede**（锚旧） | 修法（把并发 CAS 与历史溯源分开）属**放宽完整性约束**，需架构负责人拍板 | 未刷新、未 supersede |
