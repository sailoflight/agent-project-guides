...
| 2 | 决策 6 的**能力状态词表**写进哪个分发文件 | 观测输入已就绪（§13.12），但落地面=决策 2 的声明式 manifest，而决策 2 你已裁定"暂不开" | 是否开决策 2；或接受先只写进**非分发**的 ADR | 未写进任何分发面（写了也没有消费者） |
| 3 | 新术语 `declaration-observed` | 决策 8 的映射要求："EE 声明了某等级"与"该等级为真"必须分开（§13.12） | 是否进正式词表（进词表=公共契约） | 未加 |
| 4 | 决策 7 命名空间 `agent-project-guides:external:<command-name>` | 实测建议已成文（版本作字段、完整性作三元组、词法定界，§13.12） | 签字 | 未落地 |
| 5 | 证据验收合同是否升级为**机器可判**（peer review vs formal IV&V） | 现状靠角色文档 + `lib/memory.mjs` 的非作者评审约束（§13.14 审计：承重） | 是否要机器可判形态（公共契约变更） | 未改 |
...
| 8 | **P7 APG 自身块瘦身** | 2,063 B（3.0.10 后）vs 消费者 731–758 B；**已补逐条字节预算**（R3 单条占 24%，七条共 1,758 B），并**更正了这条比较**：消费者那 731–758 B 是 **v3 CLI 形态**（模板 647 B，用 383 B 的一段话覆盖了 v2 用 1,758 B 七条规则在做的事）——目标形态本仓**已在给消费方发货**（§13.18.10） | 三选一：**A** 只搬文字、保留 v2 marker 与 schema（预计省 ~1,360 B ≈ 66%，属**公共契约变更**）／**B** 接受不对称并写清（免批准）／**C** 真做 v3 自迁移（被 §12.2 实测的 `applicable: false` 挡住） | 未改块；未动分发模板、`bootstrap.bytes`、`integrity` 哈希与 `test-install.sh` 钉值 |
| 9 | `docs/memory` 记录内嵌 `project_digest` 全面变旧 | **已量化：不是 18 次腐坏，是两个批次**（16 条同一个 3.0.3 时代值 + 2 条较晚的；18/18 `state=promoted`，0 条与当前锚相符）。根因由仓内既有记录定位到行号：**同一个锚被同时当作并发 CAS 护栏与历史溯源凭据**，于是描述符一动，此后**没有任何 promoted 记录还能被 supersede**（自托管项目每次升版必中）。另发现一个同名陷阱：`project validate` 的 `project_digest`（`e4ca3608…`）与记忆锚 `projectDigest()`（`8ffb102c…`）**同名不同值**（§13.18.11） | 三选一：**A** 拆锚（唯一能让 supersede 恢复可用，属公共契约变更）／**B** 就地刷新摘要（**已定性为洗白历史，建议拒绝**）／**C** 加「纪元」字段让其可读。外加一个独立小项：给 `project validate` 那个同名字段改名（也是契约变更） | 未改 `lib/memory.mjs`、未刷新任何摘要、未动 `test-v2.mjs` 同步义务、未 supersede 任何记录 |
...
**先纠一个我自己踩的坑：有两个不同的东西都叫「project_digest」。** 我第一版拿 `apg project validate` 输出的 `project_digest`（`sha256:e4ca3608…`）当参照值，算完才去核，发现**它根本不是记忆子系统的锚**。真正的锚是 `projectDigest(descriptor) = sha256(canonicalJson(descriptor))`（`lib/memory.mjs:50-52`），当前值：

```
projectDigest(readDescriptor()) = sha256:8ffb102c930f0a0652e39420f2f93f914f7d50bd46062adce2b0b60d6e6c6638
apg project validate 的 project_digest = sha256:e4ca36081ba293fc43396ba3f8b9612d075a66bd9a278343e240d90092b66b1a
```

两者不同，而**名字一样**——任何人（包括我）照着 `project validate` 的数字去比对记忆记录，都会得出错误的相符/不符判定。这次是"用错参照但结论侥幸不变"（因为两个历史值与**两个**候选参照都不等），但这是个**该被记下来的命名陷阱**，而不是可以留着的巧合。下表用的是正确的锚 `8ffb102c…`。
...
| 选项 | 内容 | 代价 / 爆炸半径 | 性质 |
|---|---|---|---|
| **A 拆锚（把并发 CAS 与历史溯源分开）** | `supersede` 不再要求历史记录携带当前摘要；改为校验：prior 确为 `promoted`、链条单步（无二次 supersede）、prior 的溯源信息自洽；**提案本身仍必须携带当前摘要**，从而保住新内容的 CAS 性质 | 改 `lib/memory.mjs`（**分发文件**）+ 记录形状 + `test-v2.mjs:469-471` 的同步义务 + 可能的记录 schema | **公共契约变更**，需签字。这是唯一能让 supersede 恢复可用的选项 |
...
**另有一个独立的小项，别和第 9 行混在一起**：`project validate` 报的 `project_digest` 与记忆子系统的锚同名不同值。它**不属于**第 9 行的修法，但它是同一个混淆源，而且修它同样是**公共契约变更**（改的是被报告字段的名字或含义），所以也一并挂起。我建议的处理是先给它一个不含歧义的名字（例如把它明确成"描述符摘要"或"物化摘要"），而不是改记忆子系统去迁就它。
...
| 2 | **只写进非分发的 ADR** —— 能力状态词表不进分发面 | 见 §13.19.1 |
| 3 | **暂不进正式词表** —— `declaration-observed` 先在 ADR 内使用 | 见 §13.19.1 |
| 4 | **签字落地** —— 命名空间 `agent-project-guides:external:<command-name>` | 落成 **ADR 0008**；因落地面（决策 2 的声明式 manifest）仍未开，它与 #2 一致地**不进分发面**。见 §13.19.1 |
...
| 8 | **选 A** —— 只搬文字、保留 v2 marker 与 schema | 见 §13.19.5 |
| 9 | **选 A 拆锚** + 给 `project validate` 的同名字段改名 | 见 §13.19.6 |
| 10 | **补打标签** `v3.0.4`–`v3.0.10` | 见 §13.19.2 |
...
### 13.19.2 #13 落地：溯源清单已入库（含一处尺寸更正），#10 见下
