...
assert_contains "$ROOT/bootstrap/AGENTS.routing-block.md" 'assigning agent, never to the end user'
# 拓扑中立回归保护：parent/captain 层级假设不得回到分发内容（决策 5 全改，2026-09-19）
assert_not_contains "$ROOT/bootstrap/AGENTS.routing-block.md" 'parent/captain'
assert_not_contains "$ROOT/templates/SUBAGENT_ASSIGNMENT.md" 'parent/captain'
assert_not_contains "$ROOT/roles/development/REVIEWER.md" 'parent/captain'
assert_not_contains "$ROOT/roles/development/FIELD_EVALUATOR.md" 'parent/captain'
assert_contains "$ROOT/bootstrap/AGENTS.v2-block.md" '.agent-project-guides/local/suggestions'
[ -f "$ROOT/templates/SUGGESTION_BOX.md" ] || fail 'suggestion box template is missing'
assert_contains "$ROOT/templates/SUGGESTION_BOX.md" 'Type: missing-role | wrong-role | missing-mode | routing-gap | other'
assert_contains "$ROOT/procedures/PACKAGE_ADAPTATION.md" 'local/suggestions'
