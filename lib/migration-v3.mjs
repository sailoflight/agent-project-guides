...
function writeBytesAtomic(file, bytes, mode = 0o644) {
  writeFileAtomic(file, bytes, { mode, temporary: `${file}.apg-v3-rollback-${process.pid}`, durable: true });
}
...
function verifyRollbackActiveBinding(projectRoot, active, paths, env) {
  ...
  }
  {
    ...
    const expectedFiles = [...receipt.owned_tree, bytesFileRecord('local/v3-migration-receipt.json', fs.readFileSync(receiptFile))].sort((left, right) => compareCanonical(left.path, right.path));
