...
// Project-owned records and human architecture views are not generic policy.
// Shared by distribution and catalog walkers; entries may name a file or subtree.
export const DIST_EXCLUDED_DIRS = new Set([
  'docs/memory', 'docs/INDEX.md', 'docs/architecture', 'docs/modules', 'docs/verification',
]);
...
// Canonical artifacts must not depend on the host locale or ICU collation tables.
...
export function writeJsonAtomic(file, value, mode = 0o644) {
  ...
  writeFileAtomic(file, canonicalJson(value), { mode, temporary });
}
...
export function listDistributionFiles(root) {
  ...
  function visit(relative) {
    ...
    const entries = fs.readdirSync(absolute, { withFileTypes: true }).sort((a, b) => compareCanonical(a.name, b.name));
...
export function verifyFileManifest(root, manifest) {
  ...
  function visit(relative = '') {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true }).sort((left, right) => compareCanonical(left.name, right.name))) {
      const child = relative ? `${relative}/${entry.name}` : entry.name;
