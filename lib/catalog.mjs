...
export function resolveRoute(packageRoot, descriptor, { plane, role, mode, task = '', pathHint = '' } = {}) {
  ...
  const verifiedExact = exact.map((id) => loadCatalogEntry(packageRoot, catalog.find((entry) => entry.id === id)));
  ...
  return {
    ...
    suggested: searched.map((entry) => entry.id).filter((id) => !exact.includes(id)),
  };
}
