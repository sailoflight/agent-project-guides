...
import { DIST_EXCLUDED_DIRS, UserError, canonicalJson, normalizeRelative, resolveInside, sha256 } from './core.mjs';
...
function walkMarkdown(root, relative) {
  ...
  const output = [];
  for (const entry of fs.readdirSync(absolute, { withFileTypes: true }).sort((a, b) => compareCanonical(a.name, b.name))) {
    const child = `${relative}/${entry.name}`;
    // Same exclusion set as listDistributionFiles: the catalog must not reference undistributed content.
    if (DIST_EXCLUDED_DIRS.has(child)) continue;
...
function semanticIds(packageRoot) {
  ...
  ids.set('bootstrap/AGENTS.v2-block.md', 'bootstrap:agents-v2');
...
// Harness-neutral catalog IDs. `bootstrap:agents-v2` is the generic AGENTS.md v2
// bootstrap block; the DSH-branded ID used through 3.0.7 remains accepted as a read alias.
export const CATALOG_ID_ALIASES = new Map([
  ['bootstrap:dsh-v2', 'bootstrap:agents-v2'],
]);

export function normalizeCatalogId(id) {
  return CATALOG_ID_ALIASES.get(id) || id;
}
...
export function buildCatalog(packageRoot) {
  ...
  return entries.sort((left, right) => compareCanonical(left.id, right.id));
}
...
export function writeCatalog(packageRoot) {
  ...
  writeFileAtomic(file, catalogJsonl(entries), { temporary });
  return entries;
}
...
export function resolveRoute(packageRoot, descriptor, {
  ...
  for (const mandatory of descriptor.policy.mandatory) {
    const mandatoryId = normalizeCatalogId(mandatory);
    if (!catalog.some((entry) => entry.id === mandatoryId)) throw new UserError(`mandatory catalog entry is missing: ${mandatory}`, 'mandatory_missing');
    result.push(mandatoryId);
  }
  ...
  const verifyEntry = loadEntry || ((id) => loadCatalogEntry(packageRoot, catalog.find((entry) => entry.id === normalizeCatalogId(id))));
...
export function searchCatalog(entries, query, { limit = 8, kind = undefined } = {}) {
  ...
  const ranked = entries
    ...
    .sort((left, right) => right.score - left.score || compareCanonical(left.entry.id, right.entry.id));
  const results = [];
