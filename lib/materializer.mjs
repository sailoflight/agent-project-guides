...
export function buildMaterializationCandidate(projectRoot, sourceRoot, options) {
  ...
  const finalRoot = previewV3Root(projectRoot, descriptor, finalBlock, { before: rootBefore });
  const transitionRoot = previewV3Root(projectRoot, descriptor, renderTransitionBlock(descriptor), { before: rootBefore });
...
function createActive(projectRoot, candidate) {
  return {
    schema_version: 1,
    operation: 'materialize-v3',
    generation_digest: candidate.generation_digest,
    descriptor_before: snapshot(path.join(projectRoot, DESCRIPTOR)),
    guides_before_exists: fs.existsSync(path.join(projectRoot, GUIDES)),
    root_before: candidate.final_root.before,
    state: 'planned',
  };
}

function verifyFreshPreimages(projectRoot, active, candidate) {
  const descriptorNow = snapshot(path.join(projectRoot, DESCRIPTOR));
  if (active.state === 'planned' && (descriptorNow.hash !== active.descriptor_before.hash || fs.existsSync(path.join(projectRoot, GUIDES)) !== active.guides_before_exists)) {
    throw new UserError('project changed after materialization planning', 'materialization_conflict');
  }
  if (active.descriptor_before.exists || active.guides_before_exists) throw new UserError('minimal materializer supports fresh projects only', 'materialization_conflict');
...
export function applyMaterialization(projectRoot, sourceRoot, options, env = process.env) {
  ...
  if (fs.statSync(committedReceipt, { throwIfNoEntry: false })?.isFile()) {
  ...
  assertOwnedTransition(paths);
  if (!fs.existsSync(paths.active) && (fs.existsSync(path.join(projectRoot, DESCRIPTOR)) || fs.existsSync(path.join(projectRoot, GUIDES)))) {
  ...
  const candidate = buildMaterializationCandidate(projectRoot, sourceRoot, { ...options, rootBefore: existingActive?.root_before });
  const lock = localLock(projectRoot);
  try {
    let active = fs.existsSync(paths.active) ? readJson(paths.active, 'active materialization') : createActive(projectRoot, candidate);
    ...
    verifyFreshPreimages(projectRoot, active, candidate);

    if (candidate.descriptor.variant === 'shared-runtime.pinned' && active.state === 'planned') {
      const installed = installPackedRuntime(sourceRoot, env);
      installSharedLauncher(installed.root, env);
      appendJournal(paths.journal, { event: 'shared-runtime-ready', digest: candidate.descriptor.release.digest, installed: installed.installed });
    }
    ...
    if (active.state === 'transition-blocked') {
      ensureRootState(projectRoot, candidate.transition_root, [candidate.final_root.after_hash]);
      const guidesRoot = path.join(projectRoot, GUIDES);
    ...
    if (active.state === 'guides-published') {
      ...
      if (![active.descriptor_before.hash, expectedHash].includes(current.hash)) throw new UserError('descriptor changed during materialization', 'materialization_conflict');
    ...
    if (active.state !== 'ready') throw new UserError(`unknown materialization state: ${active.state}`, 'materialization_conflict');
    ...
    writeJsonDurable(path.join(projectRoot, GUIDES, 'local', 'materialization-receipt.json'), receipt);
    hardFailpoint(env, 'after-receipt-write');
    appendJournal(paths.journal, { event: 'apply-complete', generation_digest: candidate.generation_digest });
    fs.copyFileSync(paths.journal, path.join(projectRoot, GUIDES, 'local', 'materialization-journal.jsonl'));
...
export function validateMaterializedProject(projectRoot, descriptor, env = process.env) {
  ...
  if (fs.existsSync(legacyReleaseRoot)) throw new UserError('legacy generic release bytes invalidate fresh workspace containment', 'containment_conflict');
  if (descriptor.variant === 'selected-inline.none') {
