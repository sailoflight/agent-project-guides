import { fsyncDirectory, writeFileAtomic } from './files.mjs';
...
function writeBytesDurable(file, bytes, mode = 0o644) {
  writeFileAtomic(file, bytes, { mode, temporary: `${file}.tmp-${process.pid}-${Date.now()}`, durable: true });
}
...
export function buildGuideFiles(descriptor, closure, sourceState) {
  ...
  manifest.materialized_files.sort((left, right) => compareCanonical(left.path, right.path));
  ...
  try {
    ...
    if (options.migrationPreview && fs.existsSync(path.join(guidesRoot, migrationRelative))) {
      ...
      const migrationReceipt = {
        ...
        generation_digest: candidate.generation_digest,
        owned_tree: [...expectedGuideRecords(candidate), ...[...metadata.entries()].filter(([relative]) => relative !== migrationRelative).map(([relative, bytes]) => bytesRecord(relative, bytes))].sort((left, right) => compareCanonical(left.path, right.path)),
      };
