import { continuationTarget, withContextOutput, renderContext } from './context-presentation.mjs';
export { renderContext } from './context-presentation.mjs';
...
import { createCompactChoice, verifyCompactChoice } from './context-choice.mjs';

const ESTIMATOR = 'utf8-bytes/4-ceiling';
const LEGACY_CONTEXT_MAX_TOKENS = 4096;
const LEGACY_CLARIFICATION_MAX_TOKENS = 2048;
...
function validateClassifier(records, registry) {
  for (const field of ['assessment_patterns', 'implementation_delivery_patterns', 'maintenance_delivery_patterns', 'negated_maintenance_patterns', 'negated_implementation_patterns']) {
    if (!Array.isArray(registry[field]) || registry[field].some((pattern) => typeof pattern !== 'string' || !pattern)) throw new UserError(`context classifier ${field} is invalid`, 'invalid_registry');
  }
...
function rankRoles(text, allowed, registry, suppressed = new Set()) {
  return registry.roles.filter((candidate) => !suppressed.has(candidate.id)).map((candidate) => {
  ...
  }).filter((candidate) => candidate.score > 0).sort((left, right) => right.score - left.score || compareCanonical(left.id, right.id));
}
...
function rankedModes(classified, signals, fallback) {
  ...
  })).filter((candidate) => candidate.matches.length).sort((left, right) => right.matches.length - left.matches.length || compareCanonical(left.mode, right.mode));
...
function matchingPatterns(text, patterns) {
  return patterns.filter((pattern) => containsPattern(text, pattern));
}

function checkedAggregate(contextTokens, jsonTokens, format) {
  if (format === 'context') return contextTokens;
  if (format === 'json') return jsonTokens;
  return Math.max(contextTokens, jsonTokens);
}
...
function legacyClarification(options) {
  const { descriptor, kind, signals, records, registry, allowed, ranked, mandatory, viewKind, conflictReason, inferenceText, target, format } = options;
  let choiceIds;
  if (kind === 'protected') {
  ...
  const choices = selectedChoices.map((choice) => ({
    plane: choice.route.plane,
    role: choice.route.role,
    mode: choice.route.mode,
    ...choice,
    next_command: `apg context${continuationTarget(target)} --plane ${choice.route.plane} --role ${choice.route.role} --mode ${choice.route.mode}`,
  }));
  const record = {
    ...
    ...(built.unavailable.length ? { required_expansion: built.unavailable.map((choice) => `${choice.plane}/${choice.role}`) } : {}),
    union_loaded: false,
    authority_granted: false,
    token_estimate_method: ESTIMATOR,
  };
  withContextOutput(record, options);
  record.token_estimate = tokenEstimate(format === 'context' ? renderContext(record) : canonicalJson(record));
  if (record.token_estimate > LEGACY_CLARIFICATION_MAX_TOKENS) throw new UserError('clarification framing exceeds its token budget', 'context_budget_exceeded', {
    clarification_tokens: record.token_estimate,
    max_tokens: LEGACY_CLARIFICATION_MAX_TOKENS,
  ...
  enriched.budgets = { content_tokens: contentTokens, clarification_tokens: record.token_estimate, context_tokens: 0, json_tokens: 0, aggregate_tokens: 0, max_tokens: LEGACY_CONTEXT_MAX_TOKENS };
  for (let iteration = 0; iteration < 3; iteration += 1) {
    const exactTokens = tokenEstimate(canonicalJson(enriched));
    withContextOutput(enriched, options);
    const contextTokens = tokenEstimate(renderContext(enriched));
    enriched.budgets.context_tokens = contextTokens;
    enriched.budgets.json_tokens = exactTokens;
    enriched.budgets.aggregate_tokens = checkedAggregate(contextTokens, exactTokens, format);
  }
  if (enriched.budgets.aggregate_tokens > LEGACY_CONTEXT_MAX_TOKENS) throw new UserError(`clarification ${format || 'context/json'} token budget exceeded: ${enriched.budgets.aggregate_tokens} > ${LEGACY_CONTEXT_MAX_TOKENS}`, 'context_budget_exceeded', {
    mandatory_tokens: contentTokens,
    clarification_tokens: record.token_estimate,
    checked_format: format || 'context/json',
    max_tokens: LEGACY_CONTEXT_MAX_TOKENS,
  });
  return enriched;
}

function clarification(options) {
  const { descriptor, kind, signals, records, registry, allowed, ranked = [], mandatory = [], viewKind, generationKey, conflictReason, inferenceText = '', target, format } = options;
  if (descriptor.schema_version === 1) return legacyClarification({ descriptor, kind, signals, records, registry, allowed, ranked, mandatory, viewKind, conflictReason, inferenceText, target, format });
  let choiceIds;
  if (kind === 'protected') {
    const production = records.filter((record) => record.plane === 'production').map((record) => `${record.plane}/${record.id}`);
    choiceIds = [...production, 'development/maintainer'];
  ...
  if (generation && target) options = { ...options, compactTickets: Object.fromEntries(selectedChoices.map(choice => [choice.choice_id, createCompactChoice(descriptor, target, choice.choice_id, generationKey)])) };
  const choices = selectedChoices.map((choice) => ({
    ...choice,
  ...
  if (aggregateTokens > maxTokens) throw new UserError(`clarification ${format || 'context/json'} token budget exceeded: ${aggregateTokens} > ${maxTokens}`, 'context_budget_exceeded', {
    mandatory_tokens: contentTokens,
    clarification_tokens: record.token_estimate,
    checked_format: format || 'context/json',
...
export function compileContext(packageRoot, descriptor, request = {}) {
  ...
  if (sharedPinned && request.generation) {
    if (/^g[234]_/.test(request.generation)) {
      const route = verifyCompactChoice(request.generation, descriptor, request.target, request.select, request.generationKey);
      verifiedGeneration = { choices: [{ choice_id: request.select, route, route_hash: routeHash(route) }] };
    } else verifiedGeneration = verifyGeneration(request.generation, descriptor, request.generationKey);
  }
  ...
  if (request.role || request.mode || request.plane || request.select) {
    ...
    if (matchedSignals.length) return clarification({ descriptor, kind: 'protected', signals: matchedSignals, records, registry, allowed, mandatory, viewKind: view.kind, generationKey: sharedPinned ? request.generationKey : undefined, inferenceText: text, target: request.target, contextReference: request.contextReference, format: request.format });
    const assessmentSignals = matchingPatterns(text, registry.assessment_patterns);
    const implementationSignals = matchingPatterns(text, registry.implementation_delivery_patterns);
    const maintenanceSignals = matchingPatterns(text, registry.maintenance_delivery_patterns);
    const negatedMaintenanceSignals = matchingPatterns(text, registry.negated_maintenance_patterns);
    const negatedImplementationSignals = matchingPatterns(text, registry.negated_implementation_patterns);
    const effectiveMaintenanceSignals = negatedMaintenanceSignals.length ? [] : maintenanceSignals;
    const effectiveDeliverySignals = [...new Set([
      ...(negatedImplementationSignals.length ? [] : implementationSignals),
      ...effectiveMaintenanceSignals,
    ])];
    const suppressed = new Set();
    if (negatedImplementationSignals.length) {
      suppressed.add('development/developer');
      if (!effectiveMaintenanceSignals.length) suppressed.add('development/maintainer');
    }
    if (negatedMaintenanceSignals.length) suppressed.add('development/maintainer');
    const ranked = rankRoles(text, allowed, registry, suppressed);
    if (assessmentSignals.length && effectiveDeliverySignals.length) {
      const mixedIds = ['development/reviewer', 'development/developer', 'development/maintainer'];
      const mixedRanked = mixedIds.map((id) => ranked.find((candidate) => candidate.id === id)).filter(Boolean);
      return clarification({
        descriptor,
        kind: 'ordinary-ambiguity',
        signals: [...new Set([...assessmentSignals, ...effectiveDeliverySignals])],
        records,
        registry,
        allowed,
        ranked: mixedRanked,
        mandatory,
        viewKind: view.kind,
        generationKey: sharedPinned ? request.generationKey : undefined,
        conflictReason: 'mixed assessment and implementation delivery intent requires an explicit route choice',
        inferenceText: text,
    ...
    if (ranked.length === 0 || !ranked[0].available || (ranked[1] && ranked[0].score - ranked[1].score < 2)) {
      ...
      return clarification({ descriptor, kind: 'ordinary-ambiguity', signals: ranked.slice(0, 2).flatMap((candidate) => candidate.matches), records, registry, allowed, ranked, mandatory, viewKind: view.kind, generationKey: sharedPinned ? request.generationKey : undefined, conflictReason, inferenceText: text, target: request.target, contextReference: request.contextReference, format: request.format });
    }
  ...
  const maxTokens = descriptor.schema_version === 2 ? descriptor.context.max_tokens : LEGACY_CONTEXT_MAX_TOKENS;
  ...
  for (let iteration = 0; iteration < 3; iteration += 1) {
    ...
    result.budgets.aggregate_tokens = checkedAggregate(result.budgets.context_tokens, result.budgets.json_tokens, request.format);
  }
  if (result.budgets.aggregate_tokens > maxTokens) throw new UserError(`${request.format || 'context/json'} token budget exceeded: ${result.budgets.aggregate_tokens} > ${maxTokens}`, 'context_budget_exceeded', {
    ...
    checked_format: request.format || 'context/json',
    max_tokens: maxTokens,
  });
  return result;
}

export function validateContextMatrix(packageRoot, descriptor, request = {}) {
  const records = roleRecords(packageRoot);
  const allowed = new Set(selectedRoles(descriptor, records));
  const routes = records.flatMap((record) => {
    if (!allowed.has(`${record.plane}/${record.id}`)) return [];
    return allowedModes(record, descriptor.schema_version).map((mode) => routeShape(record, mode));
  });
  let maxAggregateTokens = 0;
  ...
  for (const route of routes) {
    ...
    let jsonResult;
    try {
      contextResult = compileContext(packageRoot, descriptor, { ...request, ...route, task: '', pathHint: '', format: 'context' });
      jsonResult = compileContext(packageRoot, descriptor, { ...request, ...route, task: '', pathHint: '', format: 'json' });
    } catch (error) {
      if (error instanceof UserError) throw new UserError(`context route validation failed for ${route.plane}/${route.role}/${route.mode}: ${error.message}`, error.code, {
        ...(error.details || {}),
        route,
      });
      throw error;
    }
    const contextTokens = tokenEstimate(renderContext(contextResult));
    const jsonTokens = tokenEstimate(canonicalJson(jsonResult));
    for (const [format, result] of [['context', contextResult], ['json', jsonResult]]) {
      if (result.status !== 'ready' || !result.route_resolved || result.authority_granted || result.union_loaded) {
        throw new UserError(`context route invariants failed for ${route.plane}/${route.role}/${route.mode} (${format})`, 'context_validation_failed', { route, format });
      }
      if (routeHash(route) !== result.route_hash) throw new UserError(`context route hash verification failed for ${route.plane}/${route.role}/${route.mode} (${format})`, 'context_validation_failed', { route, format });
      for (const id of descriptor.policy.mandatory) {
        if (!result.mandatory_ids.includes(id) || !result.selected_ids.includes(id)) throw new UserError(`mandatory context ID is missing from ${route.plane}/${route.role}/${route.mode} (${format}): ${id}`, 'mandatory_missing', { route, format, id });
      }
    }
    if (contextTokens !== contextResult.budgets.context_tokens || contextResult.budgets.aggregate_tokens !== contextTokens
      || jsonTokens !== jsonResult.budgets.json_tokens || jsonResult.budgets.aggregate_tokens !== jsonTokens) {
      throw new UserError(`context route output verification failed for ${route.plane}/${route.role}/${route.mode}`, 'context_validation_failed', {
        route,
        expected_context_tokens: contextTokens,
        reported_context_tokens: contextResult.budgets.context_tokens,
        context_aggregate_tokens: contextResult.budgets.aggregate_tokens,
        expected_json_tokens: jsonTokens,
        reported_json_tokens: jsonResult.budgets.json_tokens,
        json_aggregate_tokens: jsonResult.budgets.aggregate_tokens,
      });
    }
    maxAggregateTokens = Math.max(maxAggregateTokens, contextTokens, jsonTokens);
    maxContextTokens = Math.max(maxContextTokens, contextTokens);
    maxJsonTokens = Math.max(maxJsonTokens, jsonTokens);
  }
  return {
    routes_checked: routes.length,
    formats_checked: ['context', 'json'],
    max_tokens: descriptor.schema_version === 2 ? descriptor.context.max_tokens : LEGACY_CONTEXT_MAX_TOKENS,
    max_aggregate_tokens: maxAggregateTokens,
    max_context_tokens: maxContextTokens,
    max_json_tokens: maxJsonTokens,
    mandatory_ids: [...descriptor.policy.mandatory],
    route_hashes_verified: true,
    authority_granted: false,
    union_loaded: false,
  };
}
