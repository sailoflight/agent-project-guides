...
function withContextOutput(result, options = {}) {
  contextOutput.set(result, { target: options.target, reference: options.contextReference, signed: Boolean(options.generationKey) });
  return result;
}
  ...
  if (result.status === 'clarification_required') {
    ...
    for (const choice of result.choices || []) {
      if (result.generation || output.signed) {
        if (output.reference) lines.push(`  apg context${continuationTarget(output.target)} --generation ${output.reference} --select ${choice.choice_id}`);
        else lines.push(`- ${choice.choice_id}`);
      } else if (choice.next_command) lines.push(`  ${choice.next_command}`);
      else lines.push(`- ${choice.choice_id}`);
    }
    if ((result.generation || output.signed) && !output.reference) lines.push('Continuation: use the APG CLI to issue a short reference.');
    if (output.reference) lines.push('Continuation: local state; expires within 15 minutes.');
