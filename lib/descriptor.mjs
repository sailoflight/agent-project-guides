...
const TOP_KEYS = new Set(['schema_version', 'project_id', 'provider', 'integrity', 'facets', 'overlays', 'protected_effects', 'policy', 'layout']);
...
export function validateDescriptor(value, root) {
  ...
  if (value.schema_version === 2) return validateV3Descriptor(value, root);
  exactKeys(value, TOP_KEYS, 'descriptor');
  if (value.schema_version !== 1) throw new UserError('descriptor.schema_version must be 1 or 2', 'unsupported_schema');
  ...
  // Schema 1's descriptor-side anchor for the v2 bootstrap block (schema 2 has
  // the same field as a required member of its `integrity` object). Optional in
  // the schema so a descriptor written before the field existed stays readable;
  // what is *not* optional is that the bootstrap block be verifiable by at least
  // one recorded hash - the block's own integrity line or this field. That gate
  // lives in lib/bootstrap.mjs inspectBootstrap.
  if (value.integrity !== undefined) {
    const integrity = object(value.integrity, 'descriptor.integrity');
    exactKeys(integrity, new Set(['root_block_hash']), 'descriptor.integrity');
    if (!DIGEST.test(integrity.root_block_hash || '')) {
      throw new UserError('descriptor.integrity.root_block_hash must be an exact sha256 digest', 'invalid_descriptor');
    }
  }
