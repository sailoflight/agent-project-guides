...
export function contextErrorRecord(error, argv = process.argv.slice(2)) {
  ...
  if (['generation_reference_write_failed', 'generation_key_missing', 'generation_target_missing'].includes(code)) {
    result.next = 'Stop: governance context is unavailable. Report this environment error; do not retry unchanged, search for tokens, or continue repository work.';
  } else if (/^generation_|^selection_required$|^choice_unresolved$/.test(code)) {
