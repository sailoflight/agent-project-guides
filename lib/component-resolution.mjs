...
// Artifact bytes and a running service are independent observations of one component.
const packageKey = id => `package:${id}`;
const serviceKey = id => `service:${id}`;
...
function packageNodes(records, root, selected = null) {
  return new Map([...groupById(records.packages)].filter(([id]) => !selected || selected.has(id)).map(([id, entries]) => {
    ...
    return [packageKey(id), { verdict, requires: entries.length === 1 ? entries[0].requires ?? [] : [] }];
  }));
}
...
export function resolveDependencyGraph(nodes, { env = process.env, resolveReference = id => id } = {}) {
  ...
  function visit(id) {
    ...
    active.add(id);
    const checked = checkRequirements(node.requires, { env, pathValue: env.PATH, describe: dependency => visit(resolveReference(dependency))?.state === 'available' });
...
function resolveStoreGraph(nodes, records, options) {
  const packages = new Set(records.packages.map(entry => entry.id));
  // Preserve the original verify precedence for unqualified component references:
  // a declared package wins; do not fall back to service health if that package fails.
  return resolveDependencyGraph(nodes, { ...options, resolveReference: id => packages.has(id) ? packageKey(id) : serviceKey(id) });
}
...
export function verifyComponents(options = {}) {
  ...
  for (const [id] of groupById(records.services)) nodes.set(serviceKey(id), { verdict: { id, state: 'degraded', reusable: false, reason: 'service health has not been probed' }, requires: [] });
  const resolved = resolveStoreGraph(nodes, records, options);
  const packages = [...new Set(records.packages.map(e => e.id))].sort().map(id => resolved.get(packageKey(id)));
  return {
...
export async function probeComponents(options = {}) {
  ...
  for (const [id, entries] of groups) {
    ...
    nodes.set(serviceKey(id), { verdict, requires: entries[0].requires ?? [] });
  }
  const resolved = resolveStoreGraph(nodes, records, options);
  const services = [...groups.keys()].sort().map(id => resolved.get(serviceKey(id)));
