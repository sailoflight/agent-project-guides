...
const PROJECT_DIGEST = /^sha256:[0-9a-f]{64}$/;
...
export function promoteMemory(projectRoot, descriptor, id, { expectedProjectDigest, expectedTargetHash = 'missing' }, env = process.env) {
  ...
  try {
    ...
    if (existing) {
      ...
      fs.writeFileSync(temporary, promotedBytes, { mode: 0o644, flag: 'wx' });
      try {
...
export function supersedeMemory(projectRoot, descriptor, input, supersedes, env = process.env) {
  ...
  // The anchor names the descriptor epoch that produced this record, so it is
  // historical provenance: required, and required to be well formed, but
  // deliberately not compared with the current anchor. That comparison made the
  // check unsatisfiable after the first descriptor change - every promoted
  // record then carried a stale anchor and none could ever be superseded again.
  // Concurrency stays guarded where it belongs, on the proposal, which must
  // still carry the current digest in proposeMemory, reviewMemory and promoteMemory.
  if (typeof priorRecord.project_digest !== 'string' || !PROJECT_DIGEST.test(priorRecord.project_digest)) {
    throw new UserError(`superseded record has no usable provenance anchor: ${supersedes}`, 'invalid_memory');
  }
  if (
    priorRecord.schema_version !== 1 || priorRecord.state !== 'promoted' || priorRecord.project_id !== descriptor.project_id ||
    validatedPrior.id !== supersedes || review?.decision !== 'accept' ||
    ...
    throw new UserError(`superseded record is not valid reviewed project memory: ${supersedes}`, 'invalid_memory');
  }
