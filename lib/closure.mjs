...
export function buildSelectedClosure(packageRoot, descriptor, { includeContent = false } = {}) {
  ...
  const files = [...ownedPaths.values()].sort((left, right) => compareCanonical(left.path, right.path)).map(({ path: sourcePath, owner }) => {
  ...
  return {
    ...
    modules: [...modules.values()].sort((left, right) => compareCanonical(left.id, right.id)),
    files,
