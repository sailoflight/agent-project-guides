import { parseLocalEndpoint, validateHealthPath } from './service-probe.mjs';
import { IDENTITY_FIELDS } from './service-probe.mjs';
export { IDENTITY_FIELDS, IDENTITY_DEFAULTS, LOCAL_ENDPOINT, probeService } from './service-probe.mjs';
...
export function verifyEntry(dir) {
  ...
  if (!manifest || typeof manifest !== 'object' || manifest.schema_version !== 1 || manifest.kind !== PACKAGE_KIND || !Array.isArray(manifest.files)) fail('component manifest shape is invalid');
...
export function validateEntry(entry) {
  ...
  if (entry.kind === SERVICE_KIND) {
    ...
    parseLocalEndpoint(entry.endpoint, entry.transport);
    ...
    if (entry.identity !== undefined) {
      ...
      if (!entry.identity || typeof entry.identity !== 'object' || Array.isArray(entry.identity)) throw new UserError('identity must be an object', 'invalid_component');
...
export function resolvePackage(entry, root) {
  ...
  if (manifest.id !== entry.id || manifest.digest !== entry.digest) throw new UserError('component manifest identity differs from its store entry', 'component_corrupt');
...
export function checkRequirements(requires = [], { describe, env = process.env, pathValue = process.env.PATH } = {}) {
  ...
  for (const requirement of requires) {
    ...
    if (kind === 'component') {
      ...
      else unmet.push({ requirement: `component:${name}`, reason: 'the required component is missing or not available' });
    } else if (kind === 'bin') {
...
export function applyRequirements(verdict, checked) {
  if (!checked?.unmet?.length || ['conflict', 'not-installed'].includes(verdict.state)) return verdict;
  return {
    ...verdict,
    state: 'degraded',
    reusable: false,
