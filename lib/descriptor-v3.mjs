...
export function validateV3Descriptor(value, root) {
  ...
  if (!Number.isSafeInteger(context.clarification_max_tokens) || context.clarification_max_tokens < 32 || context.clarification_max_tokens > 256) throw new UserError('descriptor.context.clarification_max_tokens must be 32..256', 'invalid_descriptor');
  if (context.clarification_max_tokens >= context.max_tokens) throw new UserError('clarification budget must be smaller than aggregate context budget', 'invalid_descriptor');
  ...
  if (transitional) {
    const migration = exactObject(value.migration, 'descriptor.migration', ['state', 'from_schema_version', 'legacy_provider']);
...
export function defaultV3Descriptor({
  ...
  return {
    ...
    documents: {
      ...
      lifecycle,
      roles: roles || lifecycleRequiredRoles(lifecycle),
      profiles,
      overlays,
    },
