...
当前版本：`3.0.5`。仅 `selected-inline.none` 与 `shared-runtime.pinned` 可运行；其他模式未实现。合同见 [`docs/V3_MINIMAL_SLICE.md`](docs/V3_MINIMAL_SLICE.md)；2.0 兼容边界见 [`docs/V2_CONTRACT.md`](docs/V2_CONTRACT.md)。
...
```json
{
  ...
  "release": {"policy": "pinned", "version": "3.0.5", "digest": "sha256:<64 lowercase hex>"},
  "documents": {
...
`project validate` 预检 selected view 的全 role/mode 与两种输出预算；任一路由失败都不得返回 ready。`3.0.5` bootstrap remains prompt-level, not a host tool gate.
